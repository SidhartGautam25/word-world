...
export default function WordAnalysisSidebar({
  ...
  // Find fallback info from other songs if current song has none
  const fallbackInfo = results.find(r => r.meaning || r.example);
  
  const displayMeaning = currentWordInfo?.meaning || fallbackInfo?.meaning;
  const displayExample = currentWordInfo?.example || fallbackInfo?.example;
      ...
      <div className="h-full flex flex-col">
          ...
          {(displayMeaning || displayExample) && (
            <div className="p-8 border-b-4 border-slate-100 bg-white space-y-6">
              ...
              {displayMeaning && (
                <div className="space-y-2">
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                    {isFallback ? "Suggested Meaning" : "Contextual Meaning"}
                  </p>
              ...
              {displayExample && (
                <div className="space-y-2">
                  <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                    {isFallback ? "Sample Usage" : "Usage Example"}
                  </p>
            ...
            <div className="flex items-center justify-between">
              ...
              {!loading && (
                <div className="flex flex-col items-end">
                  <span className="text-[10px] font-bold bg-sky-100 px-2 py-1 rounded-full text-sky-700 uppercase tracking-wider mb-1">
                    {totalFrequency} Total Uses
                  </span>
                  <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">
                    Across {results.length} Songs
                  </span>
                </div>
              )}
            </div>
        ...
        <div className="p-6 bg-slate-50 border-t border-slate-100">
          <p className="text-[10px] text-slate-400 text-center uppercase tracking-widest font-black">
            Cross-Song Intelligence Active
          </p>
        </div>
      </div>
    </div>
